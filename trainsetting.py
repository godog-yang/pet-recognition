...
# 每个类别选取的图片数量
samples_per_class = setting.SAMPLES_PER_CLASS
# 图片根目录
images_root = setting.IMAGES_ROOT
# 类别->编码的映射
class_code_map = setting.CLASS_CODE_MAP

# 我们准备使用经典网络在imagenet数据集上的与训练权重，所以归一化时也要使用imagenet的平均值和标准差
image_mean = tf.constant(setting.IMAGE_MEAN)
image_std = tf.constant(setting.IMAGE_STD)
...
def train_preprocess(x, y):
    ...
    y = tf.one_hot(y, setting.CLASS_NUM)
...
for sub_images_dir in sub_images_dir_list:
    ...
    if os.path.isdir(sub_path) and sub_images_dir in setting.CLASSES:
        # 获取当前类别的编码
...
train_samples = int(total_samples * setting.TRAIN_DATASET)  # 训练集样例数
dev_samples = int(total_samples * setting.DEV_DATASET)  # 开发集样例数
...
# 打乱顺序，数据预处理，设置批大小
train_db = train_db.shuffle(10000).map(train_preprocess).batch(setting.BATCH_SIZE)
# 开发集(验证集)
...
# 数据预处理，设置批大小
dev_db = dev_db.map(dev_preprocess).batch(setting.BATCH_SIZE)
# 测试集
test_db = tf.data.Dataset.from_tensor_slices(
    (x_data[train_samples + dev_samples:], y_data[train_samples + dev_samples:]))
# 数据预处理，设置批大小
test_db = test_db.map(dev_preprocess).batch(setting.BATCH_SIZE)
